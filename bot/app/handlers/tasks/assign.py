...
async def choose_team_for_assign(callback: CallbackQuery):
    ...
    if not teams:
        await callback.message.answer("Нет активных команд для назначения.")
        return
...
async def assign_to_team(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f"✅ Задача #{task_id} назначена на команду {team.value}.")
...
async def choose_user_for_assign(callback: CallbackQuery):
    ...
    if not employees:
        await callback.message.answer("Нет доступных сотрудников для назначения.")
        return
...
async def assign_employee(callback: CallbackQuery):
    ...
    new_assignee = await get_employee(emp_id)
    ...
    await callback.message.edit_text(f"✅ Задача #{task_id} назначена на {new_assignee.full_name}.")
    await callback.answer("Назначено")
...
async def select_transfer_employee(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"✍️ Введите комментарий для передачи (или '-' для пропуска):"
    )
    await callback.answer()
...
async def process_transfer_comment(message: Message, state: FSMContext):
    ...
    new_assignee = await get_employee(to_emp_id)
    if new_assignee and new_assignee.telegram_id:
        await notify_user(
            new_assignee.telegram_id,
    ...
    await message.answer(f"✅ Задача #{task_id} передана {new_assignee.full_name}.")
    await state.clear()
