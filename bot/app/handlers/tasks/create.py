...
async def process_parking(callback: CallbackQuery, state: FSMContext):
    # Паркинг общий, корпус = 2 (условно)
...
async def process_parking_floor(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(TaskCreate.select_location_type)  # reuse
    await callback.answer()
...
async def process_description(message: Message, state: FSMContext):
    ...
    await message.answer("🔢 Введите **приоритет** (1-5, по умолчанию 3):")

@router.message(StateFilter(TaskCreate.enter_priority), F.text)
async def process_priority(message: Message, state: FSMContext):
    try:
        ...
        priority = 3
        await message.answer("⚠️ Использую приоритет 3 (по умолчанию).")
...
async def finish_photos(message: Message, state: FSMContext):
    ...
    text = (
        ...
        f"Тип локации: {data.get('location_type') or '—'}\n"
