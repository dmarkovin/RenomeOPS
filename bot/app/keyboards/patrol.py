...
def patrol_list_keyboard(patrols: List[Patrol], page: int, total_pages: int) -> InlineKeyboardMarkup:
    ...
    buttons.append([InlineKeyboardButton(text="⬅️ Назад в меню", callback_data="patrol_back_to_menu")])
...
def patrol_action_keyboard(patrol_id: int, status: str) -> InlineKeyboardMarkup:
    ...
    buttons.append([InlineKeyboardButton(text="⬅️ Назад к списку", callback_data="patrol_back_to_list")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)
